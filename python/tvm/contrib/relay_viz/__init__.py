...
class RelayVisualizer:
    ...
    def __init__(
        ...
        # graph_node_count_offset ensure each node ID is still unique across subgraph.
        graph_node_count_offset = 0

        def traverse_expr(node):
            if node in node_to_id:
                return
            node_to_id[node] = str(len(node_to_id) + graph_node_count_offset)

        for name in graph_names:
            graph_node_count_offset += len(node_to_id)
            node_to_id.clear()
